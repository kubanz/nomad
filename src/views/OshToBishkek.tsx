...
const LANG_ROUTES: Record<"en" | "ru" | "ko", string> = {
  en: "/transfers/osh-to-bishkek",
  ru: "/ru/transfers/osh-to-bishkek",
  ko: "/ko/transfers/osh-to-bishkek",
};
...
const WA_LINK = `https://wa.me/${WHATSAPP_PHONE.replace(/[^0-9+]/g, "")}?text=${encodeURIComponent(
  "Hi! I'm interested in the Osh  to  Bishkek private transfer. Please share availability and price."
)}`;
const WA_LINK_SIMPLE = `https://wa.me/${WHATSAPP_PHONE.replace(/[^0-9+]/g, "")}?text=${encodeURIComponent(
  "Hi! I need a simple transfer from Osh to Bishkek. Please share availability and price."
)}`;
...
function UpsellDivider({ lang }: { lang: "en" | "ru" | "ko" }) {
  const T = {
    ...
    body:     { en: "Long drive ahead? Make it count. The Osh  to  Bishkek route is one of the most scenic in Central Asia: a 3,200 m mountain tunnel, alpine valley with yurt camps, and the turquoise Toktogul Reservoir. Stops for lunch and photos included.", ru: "Долгая дорога? Сделайте её незабываемой. Маршрут Бишкек  to  Ош — один из самых живописных в Центральной Азии: тоннель на 3,200 м, альпийская долина с юртами и бирюзовое Токтогульское водохранилище. Остановки на обед и фото включены.", ko: "긴 여정이신가요? 의미 있게 만들어보세요. 비슈케크  to  오쉬 노선은 중앙아시아에서 가장 경치 좋은 노선 중 하나: 3,200m 산악 터널, 유르트가 있는 고산 계곡, 청록빛 톡토굴 저수지. 점심과 사진 정류장 포함." },
...
export default function OshToBishkek({ initialLang }: { initialLang?: "en" | "ru" | "ko" }) {
      ...
      <section className="relative overflow-hidden">
        ...
        <div className="relative mx-auto max-w-6xl px-4 py-10 text-white md:py-14">
          ...
          <h1 className="mt-2 text-2xl font-bold leading-tight md:text-4xl">
            {lang === "ru" ? "Бишкек  to  Ош: Приватный трансфер через перевал Тоо-Ашуу" : lang === "ko" ? "비슈케크  to  오쉬: 토오-아슈 고개 경유 프라이빗 이동" : "Osh to Bishkek Private Transfer via Too-Ashuu Pass"}
          </h1>
        ...
        <section className="mb-12">
          ...
          <div className="max-w-2xl space-y-3 text-sm leading-relaxed text-slate-600">
            {lang === "ru" ? (
              ...
              <>
                <p>Private transfer from Osh to Bishkek is the most comfortable way to cross the 620 km between northern and southern Kyrgyzstan. The route passes through the Too-Ashuu Pass (3,200 m), Suusamyr Valley, and Toktogul Reservoir.</p>
        ...
        <section className="mb-14 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
          <p className="text-sm leading-relaxed text-slate-600">
            {lang === "ru"
              ...
              : "Osh to Bishkek private transfer is the most comfortable way to reach southern Kyrgyzstan. The route passes through the famous Too-Ashuu tunnel at 3,200 m, the Suusamyr Valley, and Toktogul Reservoir. A great alternative to domestic flights for travelers who want to see the country."}
          </p>
        </section>

        {/* SEO Sections */}
        <FAQSection
          faqs={oshToBishkekFAQ[lang].length > 0 ? oshToBishkekFAQ[lang] : oshToBishkekFAQ.en}
          lang={lang}
        />

        <RelatedRoutesSection
          routes={oshToBishkekRelatedRoutes[lang].length > 0 ? oshToBishkekRelatedRoutes[lang] : oshToBishkekRelatedRoutes.en}
          lang={lang}
        />
        ...
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-3 px-4 py-6 md:flex-row">
          ...
          <nav className="flex flex-col gap-2 text-center text-xs opacity-80 md:text-left">
            <div className="flex flex-wrap justify-center gap-3 md:justify-start">
              <a href="/transfers/osh-to-bishkek" className="hover:text-emerald-600">EN: Osh  to  Bishkek</a>
            ...
            <div className="flex flex-wrap justify-center gap-3 md:justify-start">
              <a href="/ru/transfers/osh-to-bishkek" className="hover:text-emerald-600">RU: Бишкек  to  Ош</a>
            ...
            <div className="flex flex-wrap justify-center gap-3 md:justify-start">
              <a href="/ko/transfers/osh-to-bishkek" className="hover:text-emerald-600">KO: 비슈케크  to  오쉬</a>
