...
const LANG_ROUTES: Record<"en" | "ru" | "ko", string> = {
  en: "/transfers/bishkek-to-karakol",
  ru: "/ru/transfers/bishkek-to-karakol",
  ko: "/ko/transfers/bishkek-to-karakol",
};
...
function SimpleTransferBlock({ lang }: { lang: "en" | "ru" | "ko" }) {
  const T = {
    ...
    busNote: { en: "For groups of 8+ passengers, bus option available from $265. Ask when booking.", ru: "Для групп от 8 человек — автобус от $265. Уточняйте при бронировании.", ko: "8인 이상 그룹은 버스 옵션 $265부터. 예약 시 문의하세요." },
    ...
    <section className="mb-10 rounded-2xl border border-slate-200 bg-white p-6 shadow-sm md:p-8">
      ...
      <div className="mt-6 grid grid-cols-2 gap-4">
        <div className="relative rounded-2xl border-2 border-teal-400 bg-white p-5 text-center shadow-sm">
          ...
          <p className="mt-3 text-3xl font-extrabold text-slate-900">$136 <span className="text-lg">USD</span></p>
        ...
        <div className="relative rounded-2xl border-2 border-amber-400 bg-white p-5 text-center shadow-sm">
          ...
          <p className="mt-3 text-3xl font-extrabold text-slate-900">$167 <span className="text-lg">USD</span></p>
...
export default function BishkekToKarakol({ initialLang }: { initialLang?: "en" | "ru" | "ko" }) {
      ...
      <main className="mx-auto max-w-6xl px-4 py-12">
        ...
        <section className="mb-12">
          ...
          <div className="max-w-2xl space-y-3 text-sm leading-relaxed text-slate-600">
            {lang === "ru" ? (
              ...
              <>
                <p>Private transfer from Bishkek to Karakol is the most comfortable way to travel across Kyrgyzstan. The route passes through scenic mountain landscapes, the northern shore of Issyk-Kul Lake, and historic landmarks along the way.</p>
                <p>Unlike public transport, this is a door-to-door service with flexible stops and personalized itinerary.</p>
                <p>For groups of 8+ passengers, private minibus available from $265. Please contact us for custom group pricing and route planning.</p>
              </>
            )}
          </div>
        </section>
        ...
        <section className="mb-14 rounded-2xl border border-slate-100 bg-white p-6 shadow-sm">
          <p className="text-sm leading-relaxed text-slate-600">
            {lang === "ru"
              ? "Трансфер Бишкек — Каракол — самый удобный способ добраться до Иссык-Куля. Маршрут проходит через Башню Бурана, Боомское ущелье и живописный северный берег Иссык-Куля. Гибкие остановки и приватный транспорт делают этот вариант лучшим для семей и путешественников, ценящих комфорт."
              : lang === "ko"
        ...
        <div className="mx-auto flex max-w-6xl flex-col items-center justify-between gap-3 px-4 py-6 md:flex-row">
          ...
          <nav className="flex flex-col gap-2 text-center text-xs opacity-80 md:text-left">
            <div className="flex flex-wrap justify-center gap-3 md:justify-start">
              ...
              <a href="/en/manas-airport-to-karakol.html" className="hover:text-emerald-600">EN: Manas → Karakol</a>
            </div>
            <div className="flex flex-wrap justify-center gap-3 md:justify-start">
